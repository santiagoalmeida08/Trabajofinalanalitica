...
#----------------------------------------  MUESTRA DE INDICADORES AL INCIO DEL DASHBOARD 
...
# Dividir el layout en dos partes (divide la pantalla )
c1, c2= st.columns((1,1)) # Entre paréntesis se indica el tamaño de las columnas
# el uno hace la relacion del tamaño en que se divide la pantalla
# el c1, c2 hace referencia a una particion de la pantalla 
...
year = c1.slider('Año en el que se presento el suceso', 2006, 2020) # Crear variable que me almacene el año seleccionado, permite desplzarce entre años
# cuando no de le coloca paso; por defecto el paso es 1
c1.map(df[df['year']==year][['latitude', 'longitude']].dropna()) # Generar mapa
# El dropna quita nulos ya que si hay nulos en alguna variable, no se genera el mapa
# Se utiliza un filtrado teniendo en cuenta el slider para ser dinamico
...
# con el subset eliminamos los nulos en filas especificas 

c2.write(pdk.Deck( # Código para crear el mapa
    
    # Set up del mapa
    map_style='mapbox://styles/mapbox/dark-v10', # para ver todos los estilos podemos ir a map_style, pydeckk
    initial_view_state={
        'latitude' : df['latitude'].mean(),
        'longitude': df['longitude'].mean(),
        'zoom' : 9.5,
        'pitch': 50 # es que tan inclinado esta el mapa 
        }, # se debe especificar la ubicacion del mapa; se puede con el promedio o con coordenadas del google maps
    
    # Capa con información
    layers = [pdk.Layer(
        'HexagonLayer', # determina las capas, es decir como se muestra la info
        data = df2[['incident_key','latitude','longitude']],
        get_position = ['longitude','latitude'], # se pone de donde se toma las coordenadas, primero long y despues lat 
        radius = 100,
        extruded = True,# muestra en 3d la info
        elevation_scale = 4, # que tanto se elevan las barras
        elevation_range = [0,1000])] # determina el rango en que se elevan las barras
    ))
...
# agrupar las variables nulas y los datos atipicos en una sola variable 
...
df2['perp_age_group2'] = df2['perp_age_group'].replace({'<18':'1',
                              ...
                              '65+':'5',
                              'UNKNOWN': '6'})
# Creamos una variable con un orden

# Aplicar orden al DataFrame
df2= df2.sort_values('perp_age_group2',ascending = False) # ordenamos con la variable creada 
...
# se filtra el df solo en 23 y 9 y despues se agrupa sobre esas horas 
df2['hour'] = df2['hour'].astype('category')
#ña convertimos en categoria ya que sino no se distingue una de otra
...
# Hacer un checkbox
if st.checkbox('Obtener datos por fecha y barrio', False): # genera un boton para mostrar las tabñas
    ...
    df2['Fecha'] = df2['Fecha'].dt.date # quita la hora del formato fecha 
    
    # Código para convertir el DataFrame en una tabla plotly resumen
    fig = go.Figure(data=[go.Table(
        header=dict(values=list(df2.columns), # encabezado de tabla 
        fill_color='lightgrey', # color relleno encabezado
        line_color='darkslategray'), # color borde de encabezdo
